...
template<class FLOAT, size_t N>
bool
AxisAlignedBoundingBox<FLOAT, N>::intersects(AxisAlignedBoundingBox<FLOAT, N> aabb, Vector<FLOAT, N> direction) const {
    AxisAlignedBoundingBox<FLOAT, N> center_extended = AxisAlignedBoundingBox(center,
                                                                              half_edge_length + aabb.half_edge_length);
...
bool Sphere<FLOAT, N>::intersects(const Ray<FLOAT, N> &ray, Intersection_Context<FLOAT, N> &context) const {
    ...
    if (inside(ray.origin)) {
        context.normal =
                static_cast<FLOAT>(-1.0) * context.normal; // ray starts inside sphere, normal points to the inside;
    }
    return true;
}
...
template<class FLOAT, size_t N>
bool
refract(FLOAT refraction_index, Vector<FLOAT, N> normal, Vector<FLOAT, N> direction, Vector<FLOAT, N> &transmission) {
...
bool Sphere<FLOAT, N>::intersects(Sphere<FLOAT, N> sphere) const {
    ...
    Vector<FLOAT, N> v1 = (this->center);
    Vector<FLOAT, N> v2 = (sphere.center);

    FLOAT r1 = this->radius;
    FLOAT r2 = sphere.radius;

    Vector<FLOAT, N> v3 = v1 - v2;
...
bool Sphere<FLOAT, N>::inside(const Vector<FLOAT, N> p) const {
    Vector<FLOAT, N> centerPoint = this->center;

    FLOAT d = (centerPoint - p).length();
